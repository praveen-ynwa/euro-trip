...
function populateItineraryTable() {
    ...
    tripData.itinerary.forEach(item => {
        ...
        row.innerHTML = `
            ...
            <td class="p-3"><span class="text-xs text-gray-500">${new Date(item.date + 'T00:00:00Z').toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', timeZone: 'UTC' })}</span><br>${item.route}<br><span class="text-xs text-gray-500">${item.driveSummary}</span></td>
        ...
        if (mobileCards) {
            ...
            card.innerHTML = `
                ...
                <div class="mb-1 text-base font-semibold">${item.route}<span class="text-xs text-gray-500"> ${item.driveSummary}</span></div>
